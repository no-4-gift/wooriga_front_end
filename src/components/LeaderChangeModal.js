...
function LeaderChangeModal({ visible, members, onClose, changeleaderinfo }) {
  const leaderInfo = members.filter(member => member.id === changeleaderinfo);

  const leadername = leaderInfo.map((elem, index) => elem.name);
  const leaderRelation = leaderInfo.map((elem, index) => elem.relation);
  console.log("LeaderChangeModal-----------");
  console.log(members);
  console.log(changeleaderinfo);
  console.log(leadername);
  console.log(leaderRelation);
  console.log("LeaderChangeModal-----------");
  ...
  return (
    <ModalBackground visible={visible}>
      <ModalTemplate visible={visible}>
        ...
        <ChangeLeaderName>
          {leadername} | {leaderRelation}
        </ChangeLeaderName>
        <ContentText>님에게 관리자(가장)를 위임하시겠습니까?</ContentText>
        <br />
        <br />
        <SaveTxt>네</SaveTxt>
